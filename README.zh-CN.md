...
> 当前版本：`v0.1.0` 开发预览。仓库内所有业务数据均为合成示例。工作流不计算人数、不自动建群、不执行触达。
...
需要 Node.js 20+；运行 Chatflow 还需要一个 Dify 工作区。
...
1. 新建策略知识库，上传 [`knowledge/strategy`](knowledge/strategy) 中的5份文件。
...
请阅读 [`CONTRIBUTING.md`](CONTRIBUTING.md) 和 [`SECURITY.md`](SECURITY.md)。欢迎贡献新的目录适配器、确定性规则检查、Dify兼容性测试和其他行业的合成示例。
